...
async function apiGetTelemetry(deviceId, token) {
  try {
    ...
    return {};  // graceful degradation – return empty telemetry on error
  }
}
...
function mergeApiData(apiDevices, telemetryMap) {
  apiDevices.forEach(apiDev => {
    ...
    const tel = telemetryMap.get(id) || {};
    const idx = DEVICES_DATA.findIndex(d => d.id === id);

    const merged = {
      id,
      name            : apiDev.name,
      type            : apiDev.type?.includes('Flow') ? 'Flow-Meter' : 'Pump',
      location        : apiDev.label || (DEVICES_DATA[idx]?.location ?? 'Unknown'),
      active          : apiDev.active ?? false,
      lastActivityTime: apiDev.lastActivityTime ?? null,
      createdTime     : apiDev.createdTime       ?? null,
      telemetry       : tel
    };
    ...
    if (idx >= 0) {
      ...
      DEVICES_DATA.push(merged);   // new device found
    }
  });
}
